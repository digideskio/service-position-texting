...
if($_GET['title'] != '')
{
	positionsEdit($_GET['id'], $_GET['event_id'], $_GET['title'], $_GET['notify_night'], $_GET['type'], $_GET['leader_id']);
					...
					<form method="get">
						...
						<div class="form-group">
							...
							<select class="form-control" name="type" id="type">
								<option value="0" <?php if($row['type'] == 0){ echo 'selected'; } ?>>Leaders only</option>
								<option value="1" <?php if($row['type'] == 1){ echo 'selected'; } ?>>Volunteers only</option>
								<option value="2" <?php if($row['type'] == 2){ echo 'selected'; } ?>>Leaders or volunteers</option>
							</select>
						</div>
						
						<div class="form-group">
							<label for="event_id">Assigned Person</label>
